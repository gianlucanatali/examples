...
|cp| on Azure Kubernetes Service
======================================

Overview
--------

This demo shows a deployment of |cp| on Azure Kubernetes Service (AKS) leveraging |co-long| with mock data generation provided via the `Kafka Connect Datagen <https://www.confluent.io/hub/confluentinc/kafka-connect-datagen>`__.
...
The remaining steps in the **Setup** section of the instructions help you build a Kubernetes cluster on Azure Kubernetes Service (AKS).  If you already have a cluster you wish to use for the demo, you can skip to the :ref:`examples-operator-aks-base-validate` section of these instructions.

To verify the Azure account in which a new cluster will be created, run the following and verify it is the desired `Azure account <https://docs.microsoft.com/en-us/cli/azure/account?view=azure-cli-latest#az-account-set>`__:

.. include:: ../../docs/includes/az-config-list.rst


.. note:: 
	If you want more information about this command review the online documentation `Azure documentation <https://docs.microsoft.com/en-us/cli/azure/group?view=azure-cli-latest#az-group-create>`__:
	if you want to reuse an existing resource group skip this step

Create a new Resource Group for this demo you by executing the following command

.. sourcecode:: bash

    az group create --name confluent-operator-demo --location centralus
...
Create the cluster by executing the following command, first replacing the ``{{ mustache bracket }}`` values for  ``azure resource group name`` with your relevant value (use ``confluent-operator-demo`` if you created the resource group with a previous step in this demo) (estimated running time, 4 minutes). 
...
Verify that ``az`` has created the cluster properly::
    ...
    ...
    Merged "cp-examples-operator-<username>" as current context in /Users/<username>/.kube/config
...
The context should contain the proper region and cluster name.  If you used the demo ``aks-create-cluster`` function to create your cluster, the context name should have the format: ``aks_<resource-group-name>_<region>_<cp-examples-operator>-<username>``
...
The demo applies this object to the cluster with the ``kubectl apply`` command::
	...
	kubectl --context <k8s-context> -n operator apply -f <path-to-examples-repo>kubernetes/aks-base/cfg/kafka-client-secrets.yaml
...
This ConfigMap is applied to the cluster with the following command::
	...
	kubectl --context <k8s-context> -n operator apply -f <path-to-examples-repo>kubernetes/aks-base/cfg/clicks-datagen-connector-configmap.yaml
...
The job is applied to the cluster, after the Kafka Connect system is deployed with::
	...
	kubectl --context <k8s-context> -n operator apply -f <path-to-examples-repo>kubernetes/aks-base/cfg/clicks-datagen-connector-deploy-job.yaml
